...
# Teaching Assistant Assignment (Incremental)
...
The *incremental* problem is based on the *batch-based* problem and tries to fix a previously valid solution that was invalidated, e.g., by added `blocked_dates` to TAs.
Therefore, by re-optimizing the solution according to all constraints, the task is to find a similar solution compared to the previously invalidated one.
...
In contrast to the batch-based implementation, the incremental solution tries to find a solution similar to the previous solution.
...
The problem instance parameters are basically the same as the output (solution) of the batch-based TA problem.
...
- `mappings`: A dictionary of mappings from session occurrence to a list of TAs assigned to this session occurrence. Example `[{"TA_1", ["SO_1", "SO_2"]}]`

### Solution characterization

A solution to the described problem is characterized by the same parameters as the batch-based problem:
...
The solution feasibility depends on multiple constraints and are the same as in the batch-based TA problem:
...
- `mappings`: A dictionary of all previously chosen TA assignments as described above.

## Solution file

The solution file format can be structured exactly as in the batch-based TA assignment solution (e.g., in plain text (`.txt`) or in JSON (`.JSON`) format).
...
The respective objective function value is: `94`
...
In comparison of the previously chosen solution, `TA_B` cannot give `SO_A`, because they are blocked on the respective date.
The incremental approach takes this into account and re-assigns `SO_A` to `TA_D` including a penalty cost of `100` within the objective function.
