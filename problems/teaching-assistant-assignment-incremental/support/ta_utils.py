...
def get_same_assignment_cost(problem: Problem, ta: TeachingAssistant, so: SessionOccurrence) -> int:
    """
    Returns the penalty cost if the assignment in question was not chosen in the original mapping/solution.
    """
    ...
    penalty = 0
    prev_used = False
    for so_candidate in problem.orig_mappings:
        if so_candidate == so.name:
            for ta_candidate in problem.orig_mappings[so_candidate]:
                if ta_candidate == ta.name:
                    prev_used = True

    if not prev_used:
        penalty += weight_prev_assignment
